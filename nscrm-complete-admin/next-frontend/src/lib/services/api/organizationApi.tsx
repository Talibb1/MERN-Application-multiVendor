...
export const createOrganization = async (organization: CreateOrganizationInput): Promise<ApiResponse<Organization>> => {
  try {
    const response = await axiosClient.post<ApiResponse<Organization>>("organizations/createorganization", organization);
  ...
  export const getOrganizationById = async (id: number): Promise<ApiResponse<Organization[]>> => {
    try {
      const response = await axiosClient.get(`organizations/getorganizations/${id}`);
      console.log("API Response organization:", response.data);
      return response.data;
    } catch (error: any) {
  ...
  export const deleteOrganizationById = async (id: number): Promise<ApiResponse<null>> => {
    try {
      const response = await axiosClient.delete(`organizations/deleteorganizations/${id}`);
      return response.data;
    } catch (error: any) {
  ...
  export const updateUserOrganizations = async (
    organizationData: Partial<Organization>
  ): Promise<ApiResponse<Organization>> => {
    try {
      const response = await axiosClient.put(`organizations/updateuserorganizations`, organizationData);
      console.log("API Response organization34343:", response.data);
      return response.data;
    } catch (error: any) {
      const orgId = organizationData.id; 
      console.error(`Error in updateOrganization for Organization ID ${orgId}:`, error.response ? error.response.data : error.message);
      throw new Error(
        error.response ? error.response.data.message : `An error occurred while updating the organization for ID ${orgId}.`
      );
    }
  };
  ...
  export const updateOrganizationById = async (id: number, organizationData: Partial<Organization>): Promise<ApiResponse<Organization>> => {
    try {
      const response = await axiosClient.put(`organizations/updateorganizations/${id}`, organizationData);
      return response.data;
    } catch (error: any) {
